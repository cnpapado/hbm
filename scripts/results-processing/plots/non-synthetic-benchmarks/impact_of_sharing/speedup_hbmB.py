...
cols = ['magic', 'hbmA', 'hbmB_shared2', 'hbmB_shared4']
for col in cols:
    df[col] = pd.to_numeric(df[col], errors='coerce')

# Compute speedups
df['shared1'] = df['magic'] / df['hbmA']
df['shared2'] = df['magic'] / df['hbmB_shared2']
df['shared4'] = df['magic'] / df['hbmB_shared4']
...
plt.title('Effect of different numbers of data qubits sharing a single magic state (routing on lower level)')
plt.legend()
plt.grid(True)
plt.tight_layout()

# Save as 300 dpi PDF
plt.savefig('different_sharing_hbmB.pdf', dpi=300)
# plt.show()
